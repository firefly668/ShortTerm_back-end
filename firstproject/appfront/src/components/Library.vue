  ...
  <div class="home">
    ...
    <el-row>
        <el-table :data="bookList" style="width: 100%" border>
          ...
          <el-table-column prop="book_name" label="标题" min-width="100">
            <template scope="scope"> {{ scope.row.fields.book_name }} </template>
          </el-table-column>
          <el-table-column prop="add_time" label="添加时间" min-width="100">
            <template scope="scope"> {{ scope.row.fields.add_time | formatDate }} </template>
          </el-table-column>
        </el-table>
    </el-row>
  </div>
...
export default {
  ...
  methods: {
    addBook() {
      this.$http.get('http://127.0.0.1:8000/api/add_book?book_name=' + this.input).then((response) => {
